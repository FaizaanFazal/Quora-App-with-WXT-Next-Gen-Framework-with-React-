...
import './App.css';
import { store, addQuestion, removeQuestion, RootState } from '../../store';
...
// Presentational component uses Redux hooks
  ...
  useEffect(() => {
    const run = async () => {
      try {
        ...
        // Execute a scraper in the page context to avoid CSP/network issues
        const [{ result }] = await chrome.scripting.executeScript({
          ...
          func: () => {
            ...
            // Headline-like elements ending with '?'
            document.querySelectorAll('h1,h2,h3').forEach(h => {
              const t = takeText(h);
              if (t && /[?]$/.test(t)) candidates.add(t);
            });

            // Common anchor patterns and feed items
          ...
          <ul className="space-y-2">
            ...
            {questions.map((q, i) => (
              <li key={i} className="flex items-start justify-between gap-2">
                <span className="text-sm truncate-text max-w-[50px]">{q}</span>
                <button
